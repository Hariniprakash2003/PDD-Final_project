...
# spiral_test =
#  'C:/Users/Harini/Desktop/project1/spiral/testing'
#
# size = (256,256)
#
...
#         self.output_layer = nn.Sequential(
#             nn.Flatten(),
#             nn.Linear((self.h // (2 ** (len(arch) - 1))) * (self.w // (2 ** (len(arch) - 1))) * arch[-1], 256),
#             nn.LeakyReLU(),
#             nn.Linear(256, 128),
#             nn.LeakyReLU(),
#             nn.Linear(128, output_channels)
...
# with torch.no_grad():
#     for images, labels in spiral_test_dataloader:
...
#     # Train the model
#     trainer = Trainer(classification_model, optim.Adam, nn.CrossEntropyLoss(),
#                       spiral_train_dataloader, spiral_test_dataloader, {'lr': 5e-5})
#     train_losses, train_f1_scores = [], []
#     for epoch in range(35):
#         train_metric, train_loss = trainer.train_epoch(train=True)
#         train_losses.append(train_loss)
...
# class Separable_Conv(nn.Module):
#     def _init_(self, stride, input_channels, output_channels, normalization, activation, res_connect=False):
#         super(Separable_Conv, self)._init_()
#         self.stride = stride
#         self.input_channels = input_channels
#         self.output_channels = output_channels
#         self.res_connect = res_connect
...
# class Condensed_Conv(nn.Module):
#     def _init_(self, stride, input_channels, output_channels, hidden_channels, normalization, activation, res_connect=False):
#         super(Condensed_Conv, self)._init_()
#         self.stride = stride
#         self.input_channels = input_channels
#         self.output_channels = output_channels
#         self.res_connect = res_connect
...
# class Model1(nn.Module):
#     def _init_(self, input_channels, output_channels, arch, size):
#         super(Model1, self)._init_()
#         self.input_channels = input_channels
#         self.output_channels = output_channels
#         self.arch = arch
#         self.h, self.w = size
...
# class Trainer():
#     def _init_(self, model, optimizer, loss, train_dataloader, test_dataloader, optimizer_params, name):
#         self.name = name
#         self.train = train_dataloader
#         self.test = test_dataloader
#         self.device = torch.device('cuda' if torch.cuda.is_available() else "cpu")
...
#         print(f"\nModel has been trained with {n_epochs} epochs\n")
#         return [train_loss_list, test_loss_list, train_metric_list, test_metric_list]
...
#         flattened_size = (self.h // (2 ** (len(arch) - 1))) * (self.w // (2 ** (len(arch) - 1))) * arch[-1]
#         self.output_layer = nn.Sequential(
#             nn.Flatten(),
